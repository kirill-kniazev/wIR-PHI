...
class LiveImageWindow(QWidget):
    def __init__(self, camera):
        ...
        self.messageLabel.setText(f"Current position: x={self.x_coord}, {self.y_coord}, {self.z_coord}") 
    ...
    def initUI(self):
        ...
        # Create 'Live' button
        self.liveButton = QPushButton('Live')
        ...
        # Create QLineEdit for scaning area input
        self.xminInput = QLineEdit()
        self.xminInput.setPlaceholderText(f"Enter x start, µm ({self.coord_min_border}-{self.coord_max_border}):")
        self.xmaxInput = QLineEdit()
        self.xmaxInput.setPlaceholderText(f"Enter x end, µm ({self.coord_min_border}-{self.coord_max_border}):")
        self.yminInput = QLineEdit()
        self.yminInput.setPlaceholderText(f"Enter y start, µm ({self.coord_min_border}-{self.coord_max_border}):")
        self.ymaxInput = QLineEdit()
        self.ymaxInput.setPlaceholderText(f"Enter y end, µm ({self.coord_min_border}-{self.coord_max_border}):")

        # Create QLineEdit for wavenumbers input
        self.wlInput = QLineEdit()
        self.wlInput.setPlaceholderText(f"Enter ν, cm⁻¹ ({self.WAVENUM_LEFT_BORDER}-{self.WAVENUM_RIGHT_BORDER}):")
        ...
        layout.addWidget(self.wlInput)
        layout.addWidget(self.xminInput)
        layout.addWidget(self.xmaxInput)
        layout.addWidget(self.yminInput)
        layout.addWidget(self.ymaxInput)
    ...
    def on_move_click(self):
        if self.xInput.text() and self.yInput.text() and self.zInput.text():  # Check if the QLineEdit widgets are not empty
            if ( 
    self.coord_min_border <= float(self.xInput.text()) <= self.coord_max_border and 
    self.coord_min_border <= float(self.yInput.text()) <= self.coord_max_border and 
    self.coord_min_border <= float(self.zInput.text()) <= self.coord_max_border):
                    self.piezo_go_to_position(float(self.xInput.text()), float(self.yInput.text()), float(self.zInput.text()))
                    self.messageLabel.setText(f"<font color='black'>Current position: x={self.x_coord}, y={self.y_coord}, z={self.z_coord}</font>")
            else:
    ...
    def on_apply_click(self):
        if self.xminInput.text() and self.xmaxInput.text() and self.yminInput.text() and self.ymaxInput.text() and self.wlInput.text():  # Check if the QLineEdit widgets are not empty
            if (self.WAVENUM_LEFT_BORDER <= int(self.wlInput.text()) <= self.WAVENUM_RIGHT_BORDER and 
    self.coord_min_border <= float(self.xminInput.text()) <= self.coord_max_border and 
    self.coord_min_border <= float(self.xmaxInput.text()) <= self.coord_max_border and 
    self.coord_min_border <= float(self.yminInput.text()) <= self.coord_max_border and
    self.coord_min_border <= float(self.ymaxInput.text()) <= self.coord_max_border and
    float(self.xminInput.text()) < float(self.xmaxInput.text()) and 
    float(self.yminInput.text()) < float(self.ymaxInput.text())):
                    self.ir_wavenum = int(self.wlInput.text())
                    self.ff3.go_to_wavelength(self.ir_wavenum)          # Select initial laser wavelength
                    self.mirror_go_to_position_wevelength_corection(self.ir_wavenum)
                    
                    # Create array of positions
                    self.x_arr = np.arange(float(self.xminInput.text()), float(self.xmaxInput.text())+10, 5) # it was +8, 8 is the step of the piezo stage
                    self.y_arr = np.arange(float(self.yminInput.text()), float(self.ymaxInput.text())+10, 5)
                    self.positions_arr = np.zeros((len(self.x_arr) * len(self.y_arr), 2), dtype=int)
                    for j in range(len(self.y_arr)):
                        for i in range(len(self.x_arr)):
                            self.positions_arr[i+j*len(self.x_arr), 1] = self.x_arr[i]
                            self.positions_arr[i+j*len(self.x_arr), 0] = self.y_arr[j]
                    self.messageLabel.setText(f"<font color='green'>Wavenumber = {self.ir_wavenum} cm⁻¹; Measurement time is {math.ceil(len(self.positions_arr)*4/60)} min;</font>")
    ...
    def on_go_click(self):
        ...
        self.worker = Worker(self.camera, self.piezo, self.positions_arr, self.wlInput, self.sampleNameInput)
...
class Worker(QThread):
    ...
    def __init__(self, camera, piezo, positions_arr, wlInput, sampleNameInput):
        super().__init__()
        self.camera = camera
        self.piezo = piezo
    ...
    def run(self):
        st = time.time()
        cube_raw = np.empty((len(self.positions_arr),76,76), int)    # Create data cube
        cube = np.empty((len(self.positions_arr),152,152), int)      # Create data cube
        for i in range(0, len(self.positions_arr)):
            self.piezo.goxy(self.positions_arr[i, 1], self.positions_arr[i, 0])     # Move piezo stage to the next position
            time.sleep(0.5)                                                         # Wait for the piezo stage to reach the position
            self.image = self.camera.returnFinalImage_Mod(num_of_images=5456,start_image=0, Flip=True, y1=122, y2=198, x1=52, x2=128)
            cube_raw[i,:,:] = self.image                                            # Store the image in the data cube
            cube [i,:,:] = self.Intrapolation(np.shape(self.image)[0], self.image)   
            self.update_IR_image.emit(cube [i,:,:])    
            # self.update_message.emit(f"<font color='black'>Measured at x={self.piezo.get_position()[0]} µm, y={self.piezo.get_position()[1]} µm ({i}/{len(self.positions_arr)-1})</font>")
            self.update_message.emit(f"<font color='black'>Measured at x={round(self.piezo.get_position()[0], 1)} µm, y={round(self.piezo.get_position()[1], 1)} µm ({i}/{len(self.positions_arr)-1})</font>")


        self.update_message.emit(f"<font color='green'>Done in {int((time.time()-st)/60)} min</font>")
        self.piezo.goxy(self.positions_arr[0, 1], self.positions_arr[0, 0])         # Move piezo stage to the initial position
        ...
        f_name_prefix_2 = f'[x-{self.positions_arr[0, 1]}-{self.positions_arr[-1, 1]}-8][y-{self.positions_arr[0, 0]}-{self.positions_arr[-1, 0]}-8]_{self.wlInput}' # Where 8 is 8 micring fieald of view diameter

        repo_path = str(Path(__file__).resolve().parent.parent.parent)
        dir_name = f"{repo_path}\\data\\{f_name_prefix}{f_name_prefix_2}_{self.sampleNameInput.text()}" # Define directory name
        ...
        np.save(f"{dir_name}\\{f_name_prefix}{f_name_prefix_2}_raw.npy", cube_raw)      # Safe data
        np.save(f"{dir_name}\\{f_name_prefix}{f_name_prefix_2}.npy", cube)              # Safe data
        print ("data saved")
